...
def build_limits(host, partial_inventory):
    """
    Progressively build limits conf host by host
    ...
    # If limits section doesn't exists return inventory without modification
    if 'limits' not in host:
        return partial_inventory
    # For each group of the host
    for group in host['limits']:
        # If limits doesn't already exists, creating it
...
def get_inventory_recursively(raw_conf):
    ...
    for host in raw_conf['hosts']:
        ...
        for filled_pattern_host in filled_pattern_host_list:
            ...
            inventory = build_limits(filled_pattern_host, inventory)
